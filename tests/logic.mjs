...
ok('доходы, расходы и отложенное считаются раздельно', m.inc === 5000 && m.exp === 2000 && m.save === 1000,
  `${m.inc}/${m.exp}/${m.save}`);
// «Деньги пишутся одинаково»: копейки сохраняются и показываются, когда они
// есть, а у целых сумм хвоста из нулей нет.
const fmt = await p.evaluate(async () => {
  const ui = await import('/app/js/ui.js');
  return [ui.money(1500.5), ui.money(1500)].map(x => x.replace(/\u00a0/g, ' '));
});
ok('копейки не теряются', fmt[0] === '1 500,50 ₽', fmt[0]);
ok('а целые суммы без хвоста', fmt[1] === '1 500 ₽', fmt[1]);
