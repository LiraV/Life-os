...
// Текст должен говорить правду о том, что уходит, а не обещать лишнего:
// дневник уходит, если включён тумблер в чате, и это сказано отдельно.
ok('сказано, что цикл и «Внутри» не уходят', /Цикл и записи «Внутри» не отправляются никогда/i.test(txt),
  (txt.match(/Цикл и записи.{0,40}/) || [''])[0]);
ok('и что дневник — только по тумблеру', /Дневник — только если включить тумблер/i.test(txt));
ok('и перечислено, что уходит по сферам', /работа, учёба, спорт/i.test(txt));
...
// ── ошибку показываем настоящую, а не свою догадку ──────────────
{
  ...
  ok('сказано про место, а не про ключ', /не принимает запросы/i.test(said), said);
  ok('и прямо сказано, что ключ ни при чём', /ключ и права ни при чём/i.test(said), said);
  ok('никакой выдумки про модель', !/нет прав на эту модель/.test(said), said);
