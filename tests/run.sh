...
APP=$(cd .. && pwd)
# Второй сервер отдаёт приложение из подкаталога — так же, как GitHub Pages
# отдаёт его по адресу /Life-os/. Ссылку на корень делаем на месте: в репозиторий
# она бы легла с чужим абсолютным путём.
mkdir -p ./pages
[ -e ./pages/Life-os ] || ln -s "$APP" ./pages/Life-os
...
serve() {
  ...
  setsid python3 ./srv.py "$1" "$2" >/dev/null 2>&1 &
  i=0
  while [ $i -lt 50 ]; do
    curl -sf -o /dev/null "$3" && return 0
    i=$((i + 1)); sleep 0.2
...
serve 8766 ./pages http://127.0.0.1:8766/Life-os/index.html || exit 1
